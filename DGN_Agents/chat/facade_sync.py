...
from DGN_Agents.chat.sessions.context import session_context
from DGN_Agents.chat.flows.call_sync import invoke as call_invoke
from DGN_Agents.chat.flows.stream_sync import stream as stream_invoke
...
class AIChat:
    def __init__(self, system=None, model_cfg=None, **kwargs):
        self.client = Client(proxies=os.getenv("https_proxy"))
    ...
    def session(self, **kwargs):
        with session_context(self.session_store, **kwargs) as sess:
            yield sess

    def __call__(self, prompt, tools=None, **kwargs):
        return call_invoke(self, prompt, tools=tools, **kwargs)

    def stream(self, prompt, **kwargs):
        return stream_invoke(self, prompt, **kwargs)
